...
// QuerySrcUser chains the current query on the "src_user" edge.
func (bq *BillQuery) QuerySrcUser() *UserQuery {
...
// QueryDstUser chains the current query on the "dst_user" edge.
func (bq *BillQuery) QueryDstUser() *UserQuery {
	query := (&UserClient{config: bq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (bq *BillQuery) FirstIDX(ctx context.Context) int {
...
func (bq *BillQuery) IDs(ctx context.Context) (ids []int, err error) {
...
// WithSrcUser tells the query-builder to eager-load the nodes that are connected to
// the "src_user" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BillQuery) WithSrcUser(opts ...func(*UserQuery)) *BillQuery {
	query := (&UserClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withSrcUser = query
	return bq
}

// WithDstUser tells the query-builder to eager-load the nodes that are connected to
// the "dst_user" edge. The optional arguments are used to configure the query builder of the edge.
func (bq *BillQuery) WithDstUser(opts ...func(*UserQuery)) *BillQuery {
	query := (&UserClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withDstUser = query
	return bq
}
...
func (bq *BillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Bill, error) {
	...
	if query := bq.withSrcUser; query != nil {
		if err := bq.loadSrcUser(ctx, query, nodes, nil,
			func(n *Bill, e *User) { n.Edges.SrcUser = e }); err != nil {
			return nil, err
		}
	}
	if query := bq.withDstUser; query != nil {
		if err := bq.loadDstUser(ctx, query, nodes, nil,
			func(n *Bill, e *User) { n.Edges.DstUser = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BillQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Bill, init func(*Bill), assign func(*Bill, *Project)) error {
	...
	for i := range nodes {
		if nodes[i].project_bills == nil {
			continue
		}
		fk := *nodes[i].project_bills
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "project_bills" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bq *BillQuery) loadSrcUser(ctx context.Context, query *UserQuery, nodes []*Bill, init func(*Bill), assign func(*Bill, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Bill)
	for i := range nodes {
...
func (bq *BillQuery) loadDstUser(ctx context.Context, query *UserQuery, nodes []*Bill, init func(*Bill), assign func(*Bill, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Bill)
	for i := range nodes {
